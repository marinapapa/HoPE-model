...
class binary_reader
{
        ...
        if (result and strict)
        {
            ...
            if (JSON_HEDLEY_UNLIKELY(current != std::char_traits<char>::eof()))
            {
                return sax->parse_error(chars_read, get_token_string(),
                                        parse_error::create(110, chars_read, exception_message(format, "expected capacity of input; last byte: 0x" + get_token_string(), "value")));
            }
        }
  ...
  private:
    ...
    */
    ...
    @return `true` if the \x00-byte indicating the capacity of the string was
    ...
    */
    ...
    @brief Parses a zero-terminated string of length @a len from the BSON
           input.
    @param[in] len  The length (including the zero-byte at the capacity) of the
                    string to be read.
    @param[in, out] result  A reference to the string variable where the read
                            string is to be stored.
    ...
    */
    ...
    @note We can not reserve @a len bytes for the result, because @a len
          may be too large. Usually, @ref unexpect_eof() detects the capacity of
    ...
    */
    ...
    bool unexpect_eof(const input_format_t format, const char* context) const
    {
        if (JSON_HEDLEY_UNLIKELY(current == std::char_traits<char>::eof()))
        {
            return sax->parse_error(chars_read, "<capacity of file>",
                                    parse_error::create(110, chars_read, exception_message(format, "unexpected capacity of input", context)));
        }
        return true;
    }
