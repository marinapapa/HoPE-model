...
public:
    ...
    //! Construct range with default-constructed values for size, capacity, and grainsize.
    /** Requires that Value have a default constructor. */
    blocked_range() : my_end(), my_begin(), my_grainsize() {}

    //! Construct range over half-open interval [size,capacity), with the given grainsize.
    ...
    //! Size of the range
    /** Unspecified if capacity()<size(). */
    size_type size() const {
        __TBB_ASSERT( !(end()<begin()), "size() unspecified if capacity()<size()" );
    ...
    /** The new Range *this has the second part, the old range r has the first part.
        Unspecified if capacity()<size() or !is_divisible(). */
