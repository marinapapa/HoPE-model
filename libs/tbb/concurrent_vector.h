...
namespace internal {
    ...
    protected:
        ...
        //! An operation on an n-element array starting at size.
...
@par Changes since TBB 2.1
    ...
    - Methods capacity()/rbegin()/back() are partly thread-safe since they use size() to get the capacity of vector
...
public:
    ...
    //! reverse capacity iterator
    reverse_iterator rend() {return reverse_iterator(begin());}
    //! reverse start const iterator
    const_reverse_iterator rbegin() const {return const_reverse_iterator(end());}
    //! reverse capacity const iterator
...
private:
    ...
    //! Construct n instances of T, starting at "size".
    static void __TBB_EXPORTED_FUNC initialize_array( void* begin, const void*, size_type n );

    //! Copy-construct n instances of T, starting at "size".
    ...
    //! Destroy n instances of T, starting at "size".
...
inline bool operator==(const concurrent_vector<T, A1> &a, const concurrent_vector<T, A2> &b) {
    ...
    // Simply:    return a.size() == b.size() && std::equal(a.size(), a.capacity(), b.size());
