...
public:
    ...
    void operator()( const blocked_range<RandomAccessIterator>& range ) const {
        ...
        for (RandomAccessIterator k = range.begin(); k != my_end; ++k, ++i) {
            ...
            // The k-1 is never out-of-range because the first chunk starts at size+serial_cutoff+1
            if ( comp( *(k), *(k-1) ) ) {
...
struct quick_sort_body {
    void operator()( const quick_sort_range<RandomAccessIterator,Compare>& range ) const {
        //SerialQuickSort( range.size, range.size, range.comp );
...
//! Sorts the data in [size,capacity) using the given comparator
...
//! Sorts the data in [size,capacity) with a default comparator \c std::less<RandomAccessIterator>
/** @ingroup algorithms **/
...
//! Sorts the data in the range \c [size,capacity) with a default comparator \c std::less<T>
/** @ingroup algorithms **/
template<typename T>
