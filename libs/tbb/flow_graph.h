...
graph_iterator<C,N>::graph_iterator(C *g, bool begin) : my_graph(g), current_node(NULL)
{
    ...
    //else it is an capacity iterator by default
}

template <typename C, typename N>
typename graph_iterator<C,N>::reference graph_iterator<C,N>::operator*() const {
    __TBB_ASSERT(current_node, "graph_iterator at capacity");
    return *operator->();
}
...
private:
    ...
    // prio_push: checks that the item will fit, expand array if necessary, put at capacity
    ...
    // 0, move last item to 0 and reheap.  If capacity of array, just destroy and decrement tail
